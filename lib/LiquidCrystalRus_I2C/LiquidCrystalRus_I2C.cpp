...
LiquidCrystalRus_I2C :: LiquidCrystalRus_I2C(uint8_t lcd_Addr,uint8_t lcd_cols,uint8_t lcd_rows) : LiquidCrystal_I2C (lcd_Addr, lcd_cols, lcd_rows)
{
	symbol_index = 0;
	ResetAllIndex();//Сброс значений индексов (неинициализированы = 255)
}
void LiquidCrystalRus_I2C::clear()
{
	LiquidCrystal_I2C::clear();
	ResetAllIndex();
}
void LiquidCrystalRus_I2C::setCursor(uint8_t col, uint8_t row)
{
	cursor_col = col;
	cursor_row = row;
	LiquidCrystal_I2C::setCursor(cursor_col, cursor_row);
}
uint8_t LiquidCrystalRus_I2C::getCursorCol()
{
	return cursor_col;
}
uint8_t LiquidCrystalRus_I2C::getCursorRow()
{
	return cursor_row;
}
void LiquidCrystalRus_I2C::print(const char *str){
	cursor_col += LiquidCrystal_I2C::print(str);
}
void LiquidCrystalRus_I2C::print(int val, int base){
	cursor_col += LiquidCrystal_I2C::print(val, base);
}
void LiquidCrystalRus_I2C::print(unsigned int val, int base){
	cursor_col += LiquidCrystal_I2C::print(val, base);
}
void LiquidCrystalRus_I2C::print(long val, int base){
	cursor_col += LiquidCrystal_I2C::print(val, base);
}
void LiquidCrystalRus_I2C::print(unsigned long val, int base){
	cursor_col += LiquidCrystal_I2C::print(val, base);
}
void LiquidCrystalRus_I2C::print(char val, int base){
	cursor_col += LiquidCrystal_I2C::print(val, base);
}
void LiquidCrystalRus_I2C::print(unsigned char val, int base){
	cursor_col += LiquidCrystal_I2C::print(val, base);
}
void LiquidCrystalRus_I2C::print(const String &str){
	cursor_col += LiquidCrystal_I2C::print(str);
}
void LiquidCrystalRus_I2C::print(double val, int base){
	cursor_col += LiquidCrystal_I2C::print(val, base);
}
void LiquidCrystalRus_I2C::print(const wchar_t *_str){
	uint8_t rus_[8];
	int current_char	= 0;
	int size = 0;
...
void LiquidCrystalRus_I2C::CharSetToLCD(uint8_t *array, uint8_t *index)
{
	uint8_t x,y;
...
void LiquidCrystalRus_I2C::ResetAllIndex()
{
	index_rus_B=255;
	index_rus_G=255;
	index_rus_D=255;
...
wchar_t *LiquidCrystalRus_I2C::asciiutf8(unsigned char ascii)
{
