...
void High_temp_komp() {
	...
	if (*tt9_KOMP_OUT > 92) {
		...
		do {
			...
			ALARM_ZUMER();
			delay(1000);
			if (millis() - HTKomp > 3600000) break;
...
void LP_ERROR_LCD() {
	...
	if (digitalRead(presLow) == LOW) {
		//RESET_timer = millis();
	}

	if (digitalRead(presLow) == HIGH) {
